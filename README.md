...
**Status:** early development. Milestone 1 (data core: models, grouping engine, Webull + generic CSV importers, CLI), Milestone 2 (discipline engine: six built-in rules configured via [rules.yaml](rules.yaml), automatic audit on import, adherence score and weekly report data), and Milestone 3 (FastAPI routes + Next.js web UI: Dashboard, Trades, Discipline, Import, Settings) are complete. Packaging (Docker, CI) comes next — see [SPEC.md](SPEC.md) for the roadmap.
